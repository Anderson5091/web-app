  ...
  return (
    <div className="min-h-screen bg-app-bg">
      <div className="max-w-4xl mx-auto p-4 pb-24 space-y-5">
        ...
        <div>
          <button
            onClick={() => navigate("/compliance/kyc")}
            className="flex items-center gap-2 text-text-secondary hover:text-text-primary mb-4 transition-colors text-sm"
          >
            <ArrowLeft size={16} />
            Back to KYC
          </button>
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {documents.map((doc) => {
              const cfg = statusConfig[doc.status as StatusKey];
              return (
                <div key={doc.id} className="flex items-center justify-between px-5 py-4 hover:bg-card-alt transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg border ${cfg.bg} ${cfg.border}`}>
                      {React.createElement(cfg.icon, { size: 15, className: cfg.text })}
                    </div>
                    <div>
                      <p className="text-text-primary text-sm font-semibold">
                        {docTypeLabels[doc.documentType]}
                      </p>
                      <p className="text-text-subtle text-xs">Uploaded {formatDate(doc.createdAt)}</p>
                    </div>
                  </div>
                  <span
                    className={`text-[10px] uppercase font-bold px-2.5 py-1 rounded-full border ${cfg.bg} ${cfg.text} ${cfg.border}`}
                  >
                    {cfg.label}
                  </span>
                </div>
            ...
            {documents.length === 0 && (
              <div className="p-8 text-center text-text-subtle text-sm">No documents uploaded yet.</div>
            )}
          </div>

          {/* Upload row */}
          <div className="px-5 py-4 border-t border-border bg-card-alt flex items-center gap-3">
            <select
              value={docType}
              onChange={(e) => setDocType(e.target.value)}
              className="flex-1 bg-card border border-border rounded-lg px-3 py-2.5 text-sm font-medium text-text-primary outline-none focus:border-primary transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#00D6A3] to-[#0084FF] text-white rounded-lg text-sm font-bold transition-opacity hover:opacity-90 disabled:opacity-50 whitespace-nowrap"
            >
